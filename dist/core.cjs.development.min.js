...
async function findBestRoute({
    ...
    const maxRouteLevel = (
        directRoutesOnly 
        || swapMode === exports.SwapMode.ExactOut ? 1 : useLegacyTransaction ? 2 : 3
    );
...
class Jupiter {
    constructor(
        connection, // t: Solana connection object
        cluster, // n: Cluster identifier (e.g., mainnet-beta, devnet)
        tokenRouteSegments, // o: Map containing the route segments between tokens
        feeCalculator, // i: Object for calculating fees on transactions
        platformFeeAndAccounts, // a: Fee-related information including fee accounts and basis points
        quoteMintToReferrer, // s: Mapping from mint to referrer (for fees/referral systems)
        routeCacheDuration = 0, // r: Duration for which route data should be cached
        wrapUnwrapSOL = true, // u: Boolean flag to handle SOL wrapping and unwrapping
        intermediateTokens, // d: Optional set of intermediate tokens for routing
        shouldLoadSerumOpenOrders, // l: Boolean flag indicating whether to load Serum Open Orders
        addressLookupTableProvider // p: Address lookup table provider for more efficient transactions
    ) {
        // Class member variables initialized
        ...
        this.addressLookupTableProvider = addressLookupTableProvider;

        // Initialize additional class properties
        this.serumOpenOrdersPromise = void 0; // Serum open orders promise (to be initialized later)
        this.user = void 0; // User public key
        this.routeCache = new Map(); // Cache for route results
        this.routeSegmentCache = new L.default({ max: 500 }); // Cache for route segments
    }

    // Method to get deposit and fees for a specific route
    async getDepositAndFees({
        marketInfos, // e: Market information for the swap route
        userPublicKey, // t: The public key of the user
        serumOpenOrdersPromise = Promise.resolve(new Map()), // n: Serum open orders for this user
    }) {
        // Delegate to an inner function that calculates deposits and fees
        return (async ({
            connection, // e: Solana connection object
            owner, // t: Owner object representing the user
            inputMint, // n: The mint of the input token
            marketInfos, // o: The market information for the swap
            feeCalculator, // i: Object to calculate fees
            serumOpenOrdersPromise, // a: Serum open orders promise
            wrapUnwrapSOL, // s: Boolean flag for handling SOL wrapping/unwrapping
        }) => {
            // Fetch open orders for all AMMs in the route
            const openOrdersResults = await Promise.all(
                marketInfos.map(async (info) => {
                    const amm = info.amm;
                    if (amm instanceof Zt || amm instanceof sn) {
            ...
            // Get ATA instructions for intermediate and destination accounts
            const {
                userIntermediaryTokenAccountResults,
                userDestinationTokenAccountResult,
            } = await m.getAtaInstructions({
                connection,
                outputMints: marketInfos.map((info) => info.outputMint),
                owner,
                unwrapSOL: wrapUnwrapSOL,
            });

            // Determine whether SOL needs to be wrapped/unwrapped
            const hasWrapUnwrapSOL =
                !!wrapUnwrapSOL &&
                [inputMint, ...marketInfos.map((info) => info.outputMint)].some(
                    (mint) => mint.equals(m.WRAPPED_SOL_MINT)
                );

            // Return calculated transaction deposits and fees
            return m.calculateTransactionDepositAndFee({
                intermediates: userIntermediaryTokenAccountResults,
                destination: userDestinationTokenAccountResult,
                openOrders: openOrdersResults,
                hasWrapUnwrapSOL,
                feeCalculator,
            });
        })({
            ...
            owner: new m.Owner(userPublicKey),
            wrapUnwrapSOL: this.wrapUnwrapSOL,
        });
    }

    // Convenience method for getting deposit and fees for the current user
    getDepositAndFeesForUser({ marketInfos }) {
        if (this.user && this.serumOpenOrdersPromise) {
            const userOwner = new m.Owner(this.user);
            return this.getDepositAndFees({
                marketInfos,
                userPublicKey: userOwner.publicKey,
                serumOpenOrdersPromise: this.serumOpenOrdersPromise,
            });
        }
        return Promise.resolve(void 0);
    ...
    // Method to execute a swap transaction
    async exchange({
        routeInfo, // t: Information about the swap route
        userPublicKey, // n: The user's public key (if not already set)
        feeAccount, // o: Account to receive platform fees
        wrapUnwrapSOL, // i: Boolean flag for wrapping/unwrapping SOL
        blockhashWithExpiryBlockHeight, // a: Blockhash for the transaction with expiry height
        asLegacyTransaction, // s: Boolean flag to use legacy transaction format
        computeUnitPriceMicroLamports, // r: Optional unit price for compute micro lamports
    }) {
        ...
        const userOwner = new m.Owner(user);
        ...
        const shouldWrapUnwrapSOL =
            wrapUnwrapSOL != null ? wrapUnwrapSOL : this.wrapUnwrapSOL;

        // Create required accounts and instructions for the swap
        const [sourceAccount, intermediateAccounts, openOrdersInfo] =
            await Promise.all([
                inputMint.equals(m.WRAPPED_SOL_MINT) && shouldWrapUnwrapSOL
                    ? m.createAndCloseWSOLAccount({
                            connection,
                            owner: userOwner,
                            amount:
                    ...
                    : {
                            ...m.getEmptyInstruction(),
                            address: m.findAssociatedTokenAddress({
                                tokenMintAddress: inputMint,
                                walletAddress: userOwner.publicKey,
                            }),
                        },
                m.getAtaInstructions({
                    connection,
                    outputMints: routeInfo.marketInfos.map((info) => info.outputMint),
                    owner: userOwner,
                    unwrapSOL: shouldWrapUnwrapSOL,
                }),
                Promise.all(
        ...
        // Prepare the transaction data for the swap, including any intermediate tokens and Serum open orders
        const swapData = {
            intermediates: intermediateAccounts.userIntermediaryTokenAccountResults,
            destination: intermediateAccounts.userDestinationTokenAccountResult,
            openOrders: openOrdersInfo,
        };

        // Determine if Serum open orders are present
        const hasOpenOrders = swapData.openOrders.filter(Boolean).length > 0;

        // Platform fee calculation: use the provided feeAccount, or determine based on the swap mode
        const feeAccountToUse =
            feeAccount ||
            (routeInfo.swapMode === exports.SwapMode.ExactIn
                ? this.platformFeeAndAccounts.feeAccounts.get(outputMint.toBase58())
                : this.platformFeeAndAccounts.feeAccounts.get(inputMint.toBase58()));

        const platformFeeDetails = feeAccountToUse
            ? {
                    feeBps:
                        this.platformFeeAndAccounts.feeBps ||
                        Math.floor(
                            100 * routeInfo.marketInfos[lastMarketInfoIndex].platformFee.pct
                        ),
                    feeAccount: feeAccountToUse,
                }
            : void 0;

        // Build the swap instruction using the route and provided details
        const { instruction: swapInstruction } =
            await (async function buildSwapInstruction({
                user, // t: User performing the swap
                openOrdersAddresses, // n: Serum open orders addresses
                userSourceTokenAccountAddress, // o: The user's source token account address
                userIntermediateTokenAccountAddresses, // i: Intermediate token accounts (if any)
                userDestinationTokenAccountAddress, // a: Destination token account address
                routeInfo, // s: Information about the swap route
                platformFee, // r: Platform fee details (if applicable)
                computeUnitPriceMicroLamports, // u: Compute unit price in micro lamports (for transaction prioritization)
                quoteMintToReferrer, // d: Mapping of mints to referrer addresses
            }) {
                // Ensure the number of intermediate accounts matches the length of the route
                if (
                    routeInfo.marketInfos.length - 1 !==
                    userIntermediateTokenAccountAddresses.length
                ) {
                    throw new Error("Missing intermediary token account address");
                }
                ...
                // Set compute unit limits for the transaction
                setupInstructions.push(
                    e.ComputeBudgetProgram.setComputeUnitLimit({ units: 1400000 })
                );
                if (
                    ...
                    setupInstructions.push(
                        e.ComputeBudgetProgram.setComputeUnitPrice({
                            microLamports: computeUnitPriceMicroLamports,
                        })
                    );
                }

                // Determine which swap mode to use (ExactIn or ExactOut)
                if (routeInfo.swapMode === exports.SwapMode.ExactIn) {
                    // Create instructions for ExactIn swap
                    const { swapInstruction } = createExactInSwapInstruction({
                        routeInfo,
                        tokenAccounts,
                        user,
                        openOrdersAddresses,
                ...
                } else if (routeInfo.swapMode === exports.SwapMode.ExactOut) {
                    // Create instructions for ExactOut swap
                    const { swapInstruction } = createExactOutSwapInstruction({
                        routeInfo,
                        tokenAccounts,
                        user,
                        openOrdersAddresses,
                ...
                // Add cleanup instructions (e.g., close WSOL accounts if needed)
                const { signers, cleanupInstructions } = m.getEmptyInstruction();

                // Return the fully constructed swap instruction
                return {
                    instruction: {
                        setupInstructions,
                        signers,
                ...
                user: userOwner,
                openOrdersAddresses: swapData.openOrders.map((o) => o?.address),
                userSourceTokenAccountAddress: sourceAccount.address,
                userIntermediateTokenAccountAddresses: swapData.intermediates.map(
                    (intermediate) => intermediate.address
                ),
                userDestinationTokenAccountAddress: swapData.destination.address,
                routeInfo,
        ...
        // Initialize a transaction builder to execute the swap
        const transactionBuilder = new m.TransactionBuilder(
            connection,
            userOwner.publicKey,
            userOwner
        );
        ...
        transactionBuilder.addInstruction(swapInstruction);

        // Add open orders instructions if they exist
        if (hasOpenOrders) {
            swapData.openOrders.forEach((order) => {
                if (order) transactionBuilder.addInstruction(order);
            });
        }

        // Add instructions for intermediate accounts (if any)
        [...swapData.intermediates, sourceAccount, swapData.destination].forEach(
            (account) => {
                if (account) transactionBuilder.addInstruction(account);
            }
        );

        // Get the latest blockhash for the transaction if not provided
        const finalBlockhash =
            blockhashWithExpiryBlockHeight ||
            (await connection.getLatestBlockhash("confirmed"));

        // Load the address lookup tables for this transaction
        const lookupTableAccounts =
            await this.addressLookupTableProvider.getAdressLookupTableAccounts(
                routeInfo.marketInfos
                    .map(({ amm }) =>
                        amm instanceof sn ? [amm.firstAmm.id, amm.secondAmm.id] : amm.id
                    )
                    .flat()
            );

        // Build the final transaction
        const { transaction: swapTransaction } = await transactionBuilder.build(
            finalBlockhash,
            lookupTableAccounts,
            asLegacyTransaction
        );

        // Return the transaction object and lookup table accounts
        return {
            swapTransaction,
            addressLookupTableAccounts: lookupTableAccounts,
            // Provide an execute function to execute the transaction
            execute: async ({ wallet, onTransaction } = {}) => {
                return m
                    .executeTransaction({
                        connection,
                        wallet,
                        ...
                        destinationAddress: swapData.destination.address,
                        swapTransaction,
                        owner: userOwner,
                        wrapUnwrapSOL: shouldWrapUnwrapSOL,
                    })
                    .finally(() => {
                        this.routeCache.clear(); // Clear route cache after execution
                    });
            },
        };
    }
    ...
    async computeRoutes({
        ...
        const cacheKey = (
            [inputMintBase58, outputMintBase58].sort((a, b) => a.localeCompare(b)).join() 
            + onlyDirectRoutes 
            + swapMode 
            + asLegacyTransaction
        );
